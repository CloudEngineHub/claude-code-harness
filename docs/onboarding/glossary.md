...
| `Spec delta` | "This task changes the product promise" — i.e. it changes what success looks like. Read it before approving; you may need to re-check the overall plan/timeline. | 「このタスクは約束ごと自体を変える（＝完成の定義が変わる）」という注記。承認前に読み、必要なら全体計画を見直す。 |

...
### SA-10 codex-impl — 115.1〜115.3（実装本体、codex 委譲）

Lead が Bash 実行（isolated worktree + Lead diff review + cherry-pick）:
...
### SA-11 codex-review-primary — 115 の PR 前（primary、review.mode=codex）

```bash
bash scripts/codex-companion.sh review --base <cherry-pick 前の main HEAD>
```

verdict 変換は `codex-cli-only.md` の表（approve→APPROVE / needs-attention→REQUEST_CHANGES）。必要時のみ `claude -p --model opus` で Opus second opinion。
...
## 8. Session prompts（operator が新セッションに貼る 3 枚。正本）

Prompt A = 114.1〜114.5、Prompt B = HG-2 GO 後の 114.6、Prompt C = リリース後の Phase 115。
Prompt A の貼付は HG-1（事前確認 4 件）の承認行為を兼ねる。Prompt B の貼付は HG-2 の GO を兼ねる。
（各 Prompt の本文は operator への提示記録として保持。実行セッションは貼られた本文に従う）

...
Worker 実装は既完了（別系統 = claude / Codex で済んだ）、advisory pre-review を Composer に出してもらう lean path。read-only 委譲なので **worktree 不要・cherry-pick 不要・cursor 出力に対する取り込みレビュー不要**（cursor は新たな diff を生まないため）。ただし対象 diff への **brain 一次レビュー（primary verdict）は省略しない**:

1. banner + 計画: `🚀 cursor / composer-2.5-fast / review` + 「これから: composer に advisory findings 委譲 → brain 一次レビューで verdict 確定」
